...
function renderProducts(){const q=document.getElementById('searchInput')?.value||'';const list=PRODUCTS.filter(p=>(activeFilter==='all'||p.categoryKey===activeFilter)&&productMatch(p,q));const grid=document.getElementById('productGrid');grid.innerHTML=list.map(p=>`<article class="product-card reveal" data-id="${p.id}"><div class="img-wrap"><img src="${p.image}" alt="${p.name}"><span class="shine"></span></div><div class="product-body"><span class="badge">${p.badge}</span><div class="category">${p.category}</div><h3>${p.name}</h3><p>${p.short}</p><div class="tag-row">${(p.tags||[]).slice(0,3).map(t=>`<small>${t}</small>`).join('')}</div><div style="font-size:11px;color:rgba(255,180,60,.85);margin:4px 0 8px;font-weight:700" class="viewer-count" data-id="${p.id}">👁 <span>--</span> người đang xem</div><div class="price-row"><div style="display:flex;flex-direction:column;gap:2px">${p.originalPriceText?`<span style="font-size:11px;color:rgba(255,255,255,.3);text-decoration:line-through;font-weight:600;letter-spacing:.3px">${p.originalPriceText}</span>`:''}<div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap"><span class="price">${p.priceText}</span>${p.discount?`<span style="background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;font-size:10px;font-weight:900;padding:2px 8px;border-radius:999px;white-space:nowrap;letter-spacing:.3px">-${p.discount}%</span>`:''}</div></div><button class="btn btn-gold" onclick="openCheckoutById('${p.id}')">Mua ngay</button></div>${p.slots?`<div style="font-size:11px;color:#ef4444;font-weight:800;margin:3px 0 6px;letter-spacing:.2px;display:flex;align-items:center;gap:4px"><span style="display:inline-block;width:7px;height:7px;background:#ef4444;border-radius:50%;animation:blink .9s ease infinite"></span>Còn ${p.slots} slot tháng này</div>`:''}<a class="btn btn-exness" href="https://one.exnessonelink.com/intl/vi/a/20jf6d0igf" target="_blank">🎁 Đăng ký Exness — Nhận miễn phí</a><div class="product-actions"><button class="btn btn-blue" onclick="selectProduct('${p.id}')">Xem nhanh</button><a class="btn btn-blue" href="${productPageUrl(p.id)}">Trang SEO</a></div></div></article>`).join('')||'<div class="empty">Không tìm thấy sản phẩm phù hợp.</div>';observeReveal();initViewerCounts()}
