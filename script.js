...
function openCheckout(p){syncProductSelects(p.id);document.getElementById('payName').innerText=p.name;document.getElementById('payPrice').innerText=p.priceText;document.getElementById('payNote').innerText=p.name;const qrImg=document.getElementById('payQr');const qrFb=document.getElementById('qrFallback');qrImg.style.opacity='0';qrImg.style.display='block';if(qrFb)qrFb.style.display='none';qrImg.src=qrUrl(p);document.getElementById('paidNotice').style.display='none';document.getElementById('checkout').style.display='flex'}
