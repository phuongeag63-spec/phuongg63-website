...
const PRODUCTS = [
  ...
  },
  {
    ...
    tags: ['Risk:Reward', 'Trailing SL', 'Break Even', 'Partial Close', 'XAUUSD', 'Forex', 'Dashboard'],
    short: 'Hệ thống quản lý lệnh tự động theo tỷ lệ Risk:Reward — tự tính Lot, chốt lời từng phần, hòa vốn và trailing SL theo EMA. Dành cho lệnh vào tay và lệnh EA.',
    description: 'PHUONG G63 RR là EA hỗ trợ quản lý lệnh trên MetaTrader 5, hoạt động theo mô hình "bạn vào lệnh tay — EA hỗ trợ vào lệnh và quản lý". Khi bạn đặt lệnh tay kèm Stop Loss, EA tự động tính khối lượng an toàn theo % rủi ro, vào lệnh EA cùng chiều, chốt lời từng phần tại tỷ lệ RR mục tiêu, dời SL về hòa vốn và trailing Stop Loss theo EMA 100. Dashboard hiển thị trực quan ngay trên biểu đồ.',
    features: [
      'Tự tính khối lượng Lot theo % rủi ro tài khoản (mặc định 5%)',
      'Tự vào lệnh EA cùng chiều khi phát hiện lệnh tay có Stop Loss',
      'Chốt lời từng phần (80% khối lượng) khi đạt tỷ lệ Risk:Reward mục tiêu (mặc định 1:2)',
      'Tự dời SL về hòa vốn (Break Even) khi giá chạy đủ xa hướng có lợi',
      'Trailing Stop Loss theo đường EMA 100 và nến đột biến',
      'Dashboard trực quan ngay trên biểu đồ: số dư, lợi nhuận, lệnh tháng, vị trí EMA',
      'Hỗ trợ XAUUSD, XAGUSD, BTCUSD, Forex (EURUSD, GBPUSD, USDJPY...)',
      'Hoạt động bình thường trên tài khoản Cent (USC) và tài khoản VND',
    ],
    install: [
      'Thanh toán và nhắn Zalo 08222 99993 để nhận file .mq5',
      'Mở MT5 → File → Open Data Folder → MQL5 → Experts → dán file vào',
      'Mở MetaEditor (F4) → mở file vừa chép → nhấn Compile (F7) — báo 0 error là thành công',
      'Quay lại MT5, mở Navigator (Ctrl+N) → Expert Advisors → thấy Phuong G63 RR',
      'Kéo EA vào chart cặp tiền muốn giao dịch',
      'Tab Common → tích Allow Algo Trading → OK',
      'Luôn đặt Stop Loss khi vào lệnh tay để EA có thể tính toán và hỗ trợ quản lý',
    ],
    faq: [
      ['EA có tự vào lệnh không?', 'EA vào lệnh cùng chiều khi phát hiện lệnh tay của bạn có Stop Loss. Bạn vẫn là người quyết định vào lệnh đầu tiên và đặt SL — EA chỉ hỗ trợ phần quản lý sau đó.'],
      ['Nếu không đặt SL thì sao?', 'EA sẽ không kích hoạt vì không thể tính khối lượng an toàn. Luôn đặt Stop Loss khi vào lệnh tay để EA hoạt động đúng.'],
      ['Hỗ trợ những cặp tiền nào?', 'XAUUSD, XAGUSD, BTCUSD và tất cả Forex (EURUSD, GBPUSD, USDJPY...). EA tự nhận diện loại tài sản và quy đổi đơn vị "giá" phù hợp.'],
      ['Có cần VPS không?', 'Nên dùng VPS nếu giao dịch xuyên phiên, vì cơ chế chốt lời từng phần và trailing SL hoạt động theo từng tick — EA cần đang chạy trên MT5 liên tục.'],
      ['Hỗ trợ tài khoản Cent không?', 'Có. EA hoạt động bình thường trên tài khoản Cent (USC) — tự quy đổi đơn vị, không cần chỉnh sửa thêm gì.'],
    ],
    warning: 'EA là công cụ hỗ trợ quản lý lệnh, không phải robot tự động hoàn toàn. Kết quả phụ thuộc vào chiến lược vào lệnh của người dùng. Luôn test trên tài khoản Demo ít nhất 1–2 tuần trước khi dùng tài khoản thật.',
    videoTitle: '',
    videoText: '',
  },
  {
